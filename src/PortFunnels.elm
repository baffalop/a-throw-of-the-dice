...
-}
simulatedPortDict : Dict String ((JE.Value -> msg) -> JE.Value -> Cmd msg)
simulatedPortDict =
    Dict.fromList
...
-}
appTrampoline : (String -> model -> (JE.Value -> Cmd msg)) -> GenericMessage -> Funnel model msg -> State -> model -> Result String ( model, Cmd msg )
...
-}
subscriptions : (JE.Value -> msg) -> model -> Sub msg
...
-}
getCmdPort : (JE.Value -> msg) -> String -> Bool -> (JE.Value -> Cmd msg)
getCmdPort tagger moduleName useSimulator =
...
type alias FunnelDict model msg =
    ( Dict String (Funnel model msg), String -> model -> (JE.Value -> Cmd msg) )


{-| Make a `Dict` mapping `moduleName` to tagged concrete `FunnelSpec`.
-}
makeFunnelDict : List (Handler model msg) -> (String -> model -> (JE.Value -> Cmd msg)) -> FunnelDict model msg
...
-}
processValue : FunnelDict model msg -> JE.Value -> State -> model -> Result String ( model, Cmd msg )
